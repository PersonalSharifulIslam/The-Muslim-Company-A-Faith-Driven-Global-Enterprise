...
import { Plus, Package, AlertCircle, Check, Edit2 } from "lucide-react";
...
export default function AdminAssets() {
  ...
  const EMPTY = { asset_name: "", asset_type: ASSET_TYPES[0], serial_number: "", assigned_to: "", notes: "" };
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      if (editing) await api.put(`/admin/assets/${editing.id}`, form, true);
      else await api.post("/admin/assets", form, true);
  ...
  function startEdit(a: Asset) {
    setEditing(a);
    setForm({ asset_name: a.asset_name, asset_type: a.asset_type, serial_number: a.serial_number || "", assigned_to: a.assigned_to || "", notes: a.notes || "" });
    setShowForm(true);
  }
    ...
    <AdminLayout current="/admin/assets">
      <div className="p-6 max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={() => { setShowForm(true); setEditing(null); setForm(EMPTY); }} className="flex items-center gap-2 bg-secondary text-primary font-sans text-xs uppercase tracking-widest h-9 px-4">
        ...
        {showForm && (
          <form onSubmit={handleSave} className="mb-6 p-6 border border-secondary/30 bg-card grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-2 flex gap-3">
              ...
              <button type="button" onClick={() => { setShowForm(false); setEditing(null); }} className="border border-primary/20 text-primary font-sans text-xs uppercase tracking-widest h-9 px-6">Cancel</button>
            </div>
          </form>
        )}
