...
async function generatePDF(submitted: SubmittedState) {
  ...
  y = 70;
  doc.setFillColor(...CREAM); doc.rect(15, y - 5, W - 30, 24, "F");
  doc.setDrawColor(...GOLD); doc.setLineWidth(0.5); doc.rect(15, y - 5, W - 30, 24, "S");
  ...
  y += 28;
  doc.setFillColor(...DARK); doc.rect(15, y, W - 30, 20, "F");
  ...
  const sectionHeader = (title: string) => {
    doc.setFillColor(...GOLD); doc.rect(15, y, 3, 7, "F");
    doc.setFont("helvetica", "bold"); doc.setFontSize(9); doc.setTextColor(...DARK);
    doc.text(title, 22, y + 5.5);
    doc.setDrawColor(...LIGHT_GREY); doc.setLineWidth(0.3); doc.line(22, y + 8, W - 15, y + 8);
    y += 14;
  };
  ...
  const twoColRow = (l1: string, v1: string, l2: string, v2: string) => {
    const half = (W - 30) / 2;
    doc.setFont("helvetica", "bold"); doc.setFontSize(7); doc.setTextColor(...GOLD);
    doc.text(l1.toUpperCase(), 15 + 2, y); doc.text(l2.toUpperCase(), 15 + half + 5, y); y += 4;
    doc.setFont("helvetica", "normal"); doc.setFontSize(9); doc.setTextColor(...TEXT_DARK);
    doc.text(v1 || "—", 15 + 2, y); doc.text(v2 || "—", 15 + half + 5, y); y += 8;
  };
  ...
  if (submitted.form.portfolio) fieldRow("Portfolio / LinkedIn", submitted.form.portfolio);
  sectionHeader("COVER LETTER");
  fieldRow("", submitted.form.cover_letter);
  const remainingSpace = 297 - y;
  if (remainingSpace < 35) { doc.addPage(); y = 20; }
  y = Math.max(y, 265);
  doc.setFillColor(...DARK); doc.rect(0, y, W, 297 - y, "F");
  doc.setFillColor(...GOLD); doc.rect(0, y, W, 1, "F");
  doc.setFont("helvetica", "bold"); doc.setFontSize(11); doc.setTextColor(...GOLD);
  doc.text("Jazakallah Khair", W / 2, y + 10, { align: "center" });
  doc.setFont("helvetica", "normal"); doc.setFontSize(7.5); doc.setTextColor(...LIGHT_GOLD);
  ...
  doc.save(`TMC_Application_${submitted.ref.replace(/\//g, "-")}.pdf`);
}
export default function JobApply({ params }: { params: { slug: string } }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (cvFile) {
        ...
        const fileName = `${ref.replace(/\//g, "-")}-${Date.now()}.${fileExt}`;
  ...
  const handleDownloadPDF = async () => {
    if (!submitted) return;
    setPdfLoading(true);
    try { await generatePDF(submitted); } catch { }
    setPdfLoading(false);
  };
  if (submitted) {
    return (
          ...
          <div className="container mx-auto max-w-3xl">
            <motion.div initial="hidden" animate="visible" variants={fadeIn}>
              ...
              <div className="flex gap-3 flex-wrap">
                <a href="/recruitment-status"><Button className="bg-secondary text-primary hover:bg-secondary/90 rounded-none uppercase tracking-widest font-sans h-10 px-6 text-xs font-bold">Track Application Status</Button></a>
                <a href="/careers"><Button variant="outline" className="border-primary/20 text-primary hover:border-secondary rounded-none font-sans text-xs tracking-widest uppercase h-10 px-5">Back to Careers</Button></a>
              </div>
            </motion.div>
          </div>
        </div>
