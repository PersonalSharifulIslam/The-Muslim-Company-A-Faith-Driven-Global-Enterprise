...
export async function onRequestGet(context: any) {
  ...
  try {
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/blog_posts?category=eq.${encodeURIComponent(category)}&published=eq.true&select=title,slug,excerpt,created_at&order=created_at.desc`,
  ...
  const desc = `Articles in "${category}" — insights and thought leadership from The Muslim Company.`
  const url = `https://www.themuslim.company/blog/category/${encodeURIComponent(category)}`

  const postsHtml = posts.map(p => `
    <article>
      <h2><a href="https://www.themuslim.company/blog/${escapeHtml(p.slug)}">${escapeHtml(p.title)}</a></h2>
...
<head>
  <meta charset="UTF-8" />
  <title>${escapeHtml(category)} — The Muslim Company Blog</title>
  <meta name="description" content="${escapeHtml(desc)}" />
  <meta property="og:type" content="website" />
  <meta property="og:title" content="${escapeHtml(category)} Articles — The Muslim Company Blog" />
